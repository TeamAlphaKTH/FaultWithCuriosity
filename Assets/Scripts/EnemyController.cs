...
public class EnemyController:MonoBehaviour {
	...
	void Update() {
		...
		currentParanoia = player.GetComponent<Flashlight>().currentParanoia;
	...
	private void DealDamage() {
		if(distanceToPlayer < damageDistance && currentParanoia < 100) {
			player.GetComponent<Flashlight>().currentParanoia += (damageDistance - distanceToPlayer) * damageMultiplier;
		}
	}
